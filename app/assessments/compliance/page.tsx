...
import { Search, ChevronDown, Eye, FileText, Calendar, User, FileCheck } from "lucide-react"
...
export default function ComplianceAssessments() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="header-gradient px-6 py-8">
        <h2 className="text-3xl font-bold text-white">Compliance Assessments</h2>
      </div>
      ...
      <div className="px-6 py-8">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full px-4 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary appearance-none bg-background"
                >
            ...
            >
              ...
              <div className="space-y-3 text-sm text-foreground mb-4">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>Due:</strong> {assessment.due}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <User className="h-4 w-4 mt-0.5 text-primary flex-shrink-0" />
                  <span>
                    <strong>Submitted by CDO:</strong> {assessment.cdo}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <User className="h-4 w-4 mt-0.5 text-primary flex-shrink-0" />
                  <span>
                    <strong>Assigned Auditor:</strong> {assessment.auditor}
                  </span>
                </div>
                <div className="flex items-start gap-2">
               ...
               <Button
                onClick={() => {
                  if (assessment.action === "View Results") {
                    const params = new URLSearchParams({
                      name: assessment.name,
                      type: assessment.type,
                      cdo: assessment.cdo,
                      due: assessment.due,
                      submittedBy: assessment.cdo,
                      status: assessment.status,
                    })
                    router.push(`/assessments/compliance/entity-results?${params.toString()}`)
                  } else if (assessment.action === "Review Assessment") {
                    router.push("compliance/review-assessemnt")
                  }
                }}
                className="w-full bg-[#8A1538] hover:bg-[#6d1129] text-white"
                variant={assessment.action === "View Results" ? "default" : "outline"}
              >
        ...
        <div className="flex justify-center items-center gap-2 mb-8">
          <button
            ...
            className="text-foreground hover:text-primary text-sm disabled:opacity-50"
          >
            Previous
          </button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
            <button
              ...
              className={`px-3 py-1 rounded text-sm font-semibold transition ${
                currentPage === page ? "bg-[#8A1538] text-white" : "bg-muted text-foreground hover:bg-muted/80"
              }`}
            >
              {page}
            </button>
          ...
          <button className="px-3 py-1 rounded text-sm font-semibold bg-muted text-foreground hover:bg-muted/80">
            7
          </button>
          <button
